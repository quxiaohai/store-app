<template>
    <div v-show="show" class="t-wrapper" @touchmove.stop.prevent="moveHandle">
        <div class="t-mask" :class="{active:active}" @click.stop="close"></div>
        <div class="t-box" :class="{active:active}">
            <div class="t-header">
                <div class="t-header-button" @click.stop="close">取消</div>
                <div class="t-header-button" @click.stop="confirm">确认</div>
            </div>
            <div class="t-color__box"
                 :style="{ background: 'rgb(' + bgcolor.r + ',' + bgcolor.g + ',' + bgcolor.b + ')'}">
                <div ref="boxs" class="t-background boxs" @touchstart="touchstart($event, 0)"
            ...
            <div class="t-alternative">
                <div class="t-alternative__item" v-for="(item,index) in colorList" :key="index">
                    <div class="t-alternative__item-content"
...
export default {
    ...
    methods: {
        ...
        getSelectorQuery() {
            const arr = [];
            console.log('----x---', this.$refs.boxs)
            this.$refs.boxs && [].concat(this.$refs.boxs).forEach(node => {
                arr.push({
                    ...getOffset(node),
